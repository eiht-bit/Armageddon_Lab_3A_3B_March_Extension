...
SECRET_ID = os.environ.get("SECRET_ID", "lab/rds/mysqlk")
...
# Wait for SSM parameters to be available (Tokyo RDS might still be creating)
echo "Waiting for SSM parameters..."
for i in $(seq 1 30); do
  if python3 -c "import boto3; boto3.client('ssm', region_name='ap-northeast-1').get_parameter(Name='/lab/db/endpoint')" 2>/dev/null; then
...
Environment=SECRET_ID=lab/rds/mysqlk
