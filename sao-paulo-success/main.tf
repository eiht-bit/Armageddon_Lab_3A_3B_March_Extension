...
# Explanation: liberdade is the Goal.
...
# Explanation: liberdade wants the private base to call home—EIP gives the NAT a stable “addy.”
...
# Explanation: EC2 SG is liberdade’s muscle—only let in what you mean to.
...
resource "aws_vpc_security_group_ingress_rule" "liberdade_ec2_http_ingress" {
...
resource "aws_vpc_security_group_ingress_rule" "liberdade_ec2_ssh_ingress" {
  security_group_id = aws_security_group.liberdade_ec2_sg01.id
  cidr_ipv4         = "0.0.0.0/0"
  from_port         = 22
  ip_protocol       = "tcp"
...
resource "aws_vpc_security_group_egress_rule" "liberdade_ec2_egress" {
  security_group_id = aws_security_group.liberdade_ec2_sg01.id
  cidr_ipv4         = "0.0.0.0/0"
...
# resource "aws_vpc_security_group_ingress_rule" "liberdade_rds_ingress" {
...
# resource "aws_vpc_security_group_egress_rule" "liberdade_rds_egress" {
#   security_group_id = aws_security_group.liberdade_rds_sg01.id
#   cidr_ipv4         = "0.0.0.0/0"
#   ip_protocol       = "-1" # semantically equivalent to all ports
#     tags = {
...
# resource "aws_db_subnet_group" "liberdade_rds_subnet_group01" {
...
# resource "aws_db_instance" "liberdade_rds01" {
...
#   db_subnet_group_name   = aws_db_subnet_group.liberdade_rds_subnet_group01.name
#   vpc_security_group_ids = [aws_security_group.liberdade_rds_sg01.id]
...
resource "aws_iam_role_policy" "liberdade_lab1a_policy" {
  ...
  policy = jsonencode({
    ...
    Statement = [{
      ...
      Resource = "arn:aws:secretsmanager:ap-northeast-1:*:secret:lab/rds/mysqlk*"
    }]
  })
}

resource "aws_iam_role_policy" "liberdade_lab1b_policy" {
...
# resource "aws_ssm_parameter" "liberdade_db_name_param" {
#   name  = "/lab/db/name"
#   type  = "String"
#   value = var.db_name
